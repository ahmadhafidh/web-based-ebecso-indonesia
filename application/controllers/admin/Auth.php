...
class Auth extends CI_Controller {
	...
	public function log(){
		if(empty($this->session->aye)){
			redirect('admin/auth/login','refresh');
		}else{
			$this->form_validation->set_rules('password', str_replace(':', '', $this->lang->line('login_password_label')), 'required');
			$remember = (bool) $this->input->post('remember');
			$this->data['a']=$this->session->aye;
			if ($this->form_validation->run() == true){
				if ($this->ion_auth->login($this->session->aye, $this->input->post('password'), $remember)){
			...
			$this->_render_page('back/auth/login2', $this->data);
		}
	}
	...
	public function login(){
		...
		if ($this->form_validation->run() == true){
			$this->session->aye = $this->input->post('username');
			redirect('admin/auth/log', 'refresh');
		}else{
		...
		if ($this->form_validation->run() == false){
			...
			if ( $this->config->item('identity', 'ion_auth') != 'email' ){
				...
				if(empty($identity)) {
					...
					$this->session->set_flashdata('message', $this->ion_auth->errors());
					redirect("back/auth/forgot_password", 'refresh');
				}
